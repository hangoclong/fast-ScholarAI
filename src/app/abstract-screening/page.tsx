...
import { getAbstractScreeningEntries, updateScreeningStatus } from '../utils/database';
...
export default function AbstractScreeningPage() {
  ...
  // Load abstract screening entries
  const loadEntries = async () => {
    try {
      setLoading(true);
      setError(null);
      
      // Fetch entries for abstract screening
      const data = await getAbstractScreeningEntries();
      setEntries(data);
      
      // Calculate statistics
      const total = data.length;
      const pending = data.filter(entry => !entry.abstract_screening_status || entry.abstract_screening_status === 'pending').length;
      const included = data.filter(entry => entry.abstract_screening_status === 'included').length;
      const excluded = data.filter(entry => entry.abstract_screening_status === 'excluded').length;
      const maybe = data.filter(entry => entry.abstract_screening_status === 'maybe').length;
      
      setStats({ total, pending, included, excluded, maybe });
    } catch (err: any) {
      console.error('Error loading abstract screening entries:', err);
  ...
  const handleScreeningAction = async (id: string, status: ScreeningStatus, notes?: string) => {
    try {
      ...
      setStats(prev => {
        ...
        const prevStatus = entries.find(e => e.ID === id)?.abstract_screening_status || 'pending';
    ...
    <Layout className="min-h-screen">
      ...
      <Content className="p-6">
        <div className="bg-white p-6 rounded-md shadow-sm">
          <Title level={4}>Abstract Screening</Title>
          ...
          <Divider orientation="left">Entries for Abstract Screening</Divider>
          ...
          {loading ? (
            ...
            <LiteratureTable 
              ...
              showAllEntries={false} // Only show entries that passed title screening
